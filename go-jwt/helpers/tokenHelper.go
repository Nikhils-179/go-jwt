...
// InvalidateToken takes a token string and invalidates it by removing it from the user's document.
func InvalidateToken(ctx context.Context, tokenString string) error {
	// Create a filter to find the user by token
	filter := bson.M{"token": tokenString}

	// Update the user's document to remove the token
